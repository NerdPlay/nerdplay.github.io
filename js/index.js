...
var AudioPlayer = (function() {
  ...
  function init(options) {
    ...
    if(apActive || player === null) {
      return 'Nerdcast está reproduzindo';
    }
    ...
    if(settings.autoPlay) {
      ...
      plLi[index].classList.add('pl-list--current');
      var namec = playList[index].product_name + ' ' +  playList[index].episode + ' : ' + playList[index].title;
      
      var desc = playList[index].description;
      var rep = desc.replace("<p>", "");
      var description = rep.replace("</p>", "");
      var iconss = playList[index].image;
      var icons = iconss.replace(".jpg", "-100x75.jpg");
      var len = playList[index].insertions.length;
      var insert = playList[index].insertions;
      $('.insertions').html('');
      for (var i = 0; i < len; i++) {
        $('.insertions').append(
        '<div class="promos id-' + i +'">' +
  ...
  function beforeUnload(evt) {
    if(!audio.paused) {
      var message = 'Nerdcast está reproduzindo - Tem certeza que quer sair?';
 ...
 */
    function renderPL() {
      ...
      playList.forEach(function(item, i) {
        html.push(
          tplList.replace('{count}', i).replace('{title}', item.product_name + ' ' +  item.episode + ' : ' + item.title)
        );
      });
 ...
 */
  function play(currentIndex) {
    ...
    changeDocumentTitle(playList[index].product_name + ' ' +  playList[index].episode + ' : ' + playList[index].title);
  ...
  function jump() {
    var pula = playList[index]['jump-to-time']['end-time'];
    audio.currentTime = pula;
  }
  ...
  function timeUpdate() {
    ...
    //console.log(playList[index].insertions);
...
/* Destroy method. Clear All */
  function destroy() {
    ...
    jumpBtn.removeEventListener('click', jump, false);    
...
var https = 'http://192.168.1.80/nerdplay/nerdcasts.json';
